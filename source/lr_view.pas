...
uses
  ...
  ExtCtrls, Buttons, StdCtrls, Menus, GraphType, LCLType, LCLIntf, ComCtrls,
  LazUTF8, LR_Const, PrintersDlgs;
...
type
  ...
  public
    ...
    property ZoomCb: Double read GetZoom write SetZoom;
  published
  ...
  TfrPreviewForm = class(TForm)
    ...
    BtPgFirst: TToolButton;
    PgUp: TToolButton;
    PgDown: TToolButton;
    BtPgLast: TToolButton;
    ...
    procedure EdCurPageEditingDone(Sender: TObject);
...
var
  ...
  h: HRGN;
begin
  if not Preview.PaintAllowed then
  Exit;
...
begin
  ...
  PgUp.Hint    := sPreviewFormPrevPage;
  PgDown.Hint  := sPreviewFormNextPage;
  BtPgFirst.Hint  := sPreviewFormFirsPage;
  BtPgLast.Hint   := sPreviewFormLastPage;
...
procedure TfrPreviewForm.ZoomCbChange(Sender: TObject);
var
  i : Integer;
  c : String;
begin
...
begin
  ...
  begin
    ...
    ZoomCb.Visible := pbZoom in TfrReport(Doc).PreviewButtons;
...
begin
  ...
  begin
    ...
    if c is TToolButton then
      //TToolButton(c).Enabled := Value; //** GrayedInactive := Value;
  end;
end;
...
procedure TfrPreviewForm.EdCurPageEditingDone(Sender: TObject);
var
  i : Integer = 0;
begin
  if EMFPages = nil then
     Exit;
  i:= StrToInt(EdCurPage.Text);
  if (i > 0) AND ( i <= TfrEMFPages(EMFPages).Count) then
    CurPage:= i
  else
    CurPage:= 1;
  ShowPageNum;
...
begin
  if EMFPages = nil then Exit;
  lbNPage.Caption   :=  sFrom + ' ' + IntToStr(TfrEMFPages(EMFPages).Count);
  EdCurPage.Text    :=  IntToStr(CurPage);
  if TfrEMFPages(EMFPages).Count > 1 then
    begin
      EdCurPage.Enabled := True;
      If CurPage = TfrEMFPages(EMFPages).Count then
        begin
          PgDown.Enabled    := False;
          BtPgLast.Enabled  := False;
          PgUp.Enabled      := True;
          BtPgFirst.Enabled := True;
        end
      else if CurPage = 1 then
        begin
          PgDown.Enabled    := True;
          BtPgLast.Enabled  := True;
          PgUp.Enabled      := False;
          BtPgFirst.Enabled := False;
        end
      else
      begin
        PgDown.Enabled    := True;
