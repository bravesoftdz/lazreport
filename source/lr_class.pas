...
type
  ...
  TfrStretchType    = (stCurrentHeight,stBandHeight,stMaxHeight);
  TfrPositionType   = (ptFloat,ptRelativeToFixTop,ptRelativeToFixBottom);
  TfrPrintOption    = (poPRepeatValue,
                       poPInFirstWholeBand,
                       poPWhenDetailOverflows);
  ...
  TfrPrintStrings=Class(TStringList);
  ...
  private
    ...
    FStretchType: TfrStretchType;
    fVisible: Boolean;
    fUpdate : Integer;
    FPositionType: TfrPositionType;
    FPrintOptions : TfrPrintOptions;
    FPrintWhenExpression : TfrPrintStrings;

    procedure SetMemo(const AValue: TfrMemoStrings);
    procedure SetPositionType(AValue: TfrPositionType); virtual;
    procedure SetPrintOptions(AValue: TfrPrintOptions);
    procedure SetPrintWhenExpression(const AValue: TfrPrintStrings);
    procedure SetScript(const AValue: TfrScriptStrings);
    procedure SetStretchType(AValue: TfrStretchType); virtual;
  protected
    ...
    function GetPrintOptions : TfrPrintOptions; virtual;
  ...
  public
    ...
    property StretchType : TfrStretchType read FStretchType write SetStretchType;
    property PositionType : TfrPositionType read FPositionType write SetPositionType;
    property PrintWhenExpression : TfrPrintStrings read FPrintWhenExpression write SetPrintWhenExpression;
    property PrintOptions : TfrPrintOptions read GetPrintOptions write SetPrintOptions;
  end;
  ...
  public
    ...
    property DataSet   : String read GetDataSet write SetDataSet;
  published
  ...
  published
    ...
    property PrintWhenExpression;
    property StretchType;
    property PositionType;
  end;

  TfrTextArea = class(TfrCustomMemoView)
  published
    property Cursor;
    property Font;
    property Alignment;
  ...
  private
    ...
    procedure SetoldBottom(AValue: Integer);
    procedure SetoldLeft(AValue: Integer);
    procedure SetoldRight(AValue: Integer);
    procedure SetoldTop(AValue: Integer);
  ...
  published
    ...
    property oldRight  : Integer read FoldRight write SetoldRight ;
    property oldBottom : Integer read FoldBottom write SetoldBottom;
  end;
...
begin
  Result := nil;
  case Typ of
    gtMemo, gtTextArea:      Result := TfrMemoView.Create(AOwnerPage);
...
begin
  ...
  if frbTop in Frames    then
    Dec(bx1, wx2);
  if frbLeft in Frames   then
    Dec(by1, wy2);
  if frbBottom in Frames then
    Inc(bx, wx1);
  if frbRight in Frames  then
    Inc(by, wy1);
...
begin
  ...
  if (DocMode = dmPrinting) then
    if (FillColor = clNone) and (not FFindHighlight) then
    Exit;
...
begin
  ...
  FFramecolor := StringToColor(XML.GetValue(Path+'Frames/FrameColor/Value', 'clBlack')); // TODO Check default
   ...
   DataSet:= XML.GetValue(Path+'DataSets/DataSet/Value','');

   DataField:= XML.GetValue(Path+'DataSets/DataField/Value','');

   Stretched:= StrToBool(XML.GetValue(Path+'Stretches/Stretched/Value','0'));
   RestoreProperty('StretchType',XML.GetValue(Path+'Stretches/StretchType/Value','stCurrentHeight'));

  RestoreProperty('PositionType',XML.GetValue(Path+'PositionType/Value','ptFloat'));

   PrintWhenExpression.Text := XML.GetValue(Path+'Print/PrintWhenExpression/Value','');

   s :=  XML.GetValue(Path+'Print/PrintOptions/Value','poPRepeatValue');
   RestoreProperty('PrintOptions',s);
...
begin
  ...
  if IsPublishedProp(self,'Frames') then
   ...
   if IsPublishedProp(self,'DataSet') then
    XML.SetValue(Path+'DataSets/DataSet/Value', GetSaveProperty('DataSet'));

   if IsPublishedProp(self,'DataField') then
    XML.SetValue(Path+'DataSets/DataField/Value', GetSaveProperty('DataField'));

   if IsPublishedProp(self,'Stretched') then
    XML.SetValue(Path+'Stretches/Stretched/Value', GetSaveProperty('Stretched'));
   if IsPublishedProp(self,'StretchType') then
    XML.SetValue(Path+'Stretches/StretchType/Value', GetSaveProperty('StretchType'));

    if IsPublishedProp(self,'PositionType') then
     XML.SetValue(Path+'PositionType/Value', GetSaveProperty('PositionType'));

    if IsPublishedProp(self,'PrintOptions') then
     XML.SetValue(Path+'Print/PrintOptions/Value', GetSaveProperty('PrintOptions'));
    if IsPublishedProp(self,'PrintWhenExpression') then
      XML.SetValue(Path+'Print/PrintWhenExpression/Value', GetSaveProperty('PrintWhenExpression'));
...
procedure TfrView.SetDataSet(AValue: String);
begin
...
function TfrView.GetDataSet: String;
begin
...
begin
 ...
 RestoreProperty('HideDuplicates',XML.GetValue(Path+'Visibility/HideDuplicates/Value','0'));
 RestoreProperty('HideZeroValues',XML.GetValue(Path+'Visibility/HideZeroValues/Value','0'));
...
begin
  ...
  XML.SetValue(Path+'Visibility/HideDuplicates/Value', GetSaveProperty('HideDuplicates'));
  XML.SetValue(Path+'Visibility/HideZeroValues/Value',GetSaveProperty('HideZeroValues'));
...
procedure TfrPage.SetMargins;
begin
  with Margins do
  begin
    if (Left = 0) and (Right = 0) and ( Top = 0) and ( Bottom = 0) then
      begin
        AsRect := Rect(FR_U_CM,FR_U_CM,FR_U_CM,FR_U_CM);
      end;
  end;
end;
  ...
  with PrnInfo do
    if UseMargins then
      if Margins.Bottom = 0 then
        Result:=Ofy+Ph
      else if Self.ZoomPage then
        Result := Self.Height -Margins.Bottom
      else
    ...
    if UseMargins then
      begin
        if Margins.Right = 0 then
          ...
          Result := Self.Width - Margins.Right
        else
...
procedure TfrPage.SetZoomPage(AValue: Boolean);
begin
...
begin
  ...
  begin
    ...
    XML.SetValue(aSubPath+'Category/Value', Category);
...
begin
  ...
  begin
    ...
    begin
      ...
      Category := XML.GetValue(aSubPath+'Category/Value', ''); // TODO check default value
...
procedure TfrObject.SetPositionType(AValue: TfrPositionType);
begin
...
procedure TfrObject.SetPrintOptions(AValue: TfrPrintOptions);
begin
  if FPrintOptions=AValue then Exit;
  FPrintOptions:=AValue;
end;

procedure TfrObject.SetPrintWhenExpression(const AValue: TfrPrintStrings);
begin
  if FPrintWhenExpression = AValue then
     Exit;
  FPrintWhenExpression.Assign(AValue);
end;
...
function TfrObject.GetPrintOptions: TfrPrintOptions;
begin
...
procedure TfrObject.SetStretchType(AValue: TfrStretchType);
begin
  if FStretchType=AValue then
  Exit;
  BeforeChange;
  FStretchType:= AValue ;
  AfterChange;
...
procedure TfrObject.BeforeChange;
begin
  if (frDesigner<>nil) and (fUpdate=0) and (DocMode=dmDesigning) then
    frDesigner.BeforeChange;
end;

procedure TfrObject.AfterChange;
begin
   if (frDesigner<>nil) and (fUpdate=0) and (DocMode=dmDesigning) then
    frDesigner.AfterChange;
end;
...
begin
  ...
  fScript:=TfrScriptStrings.Create;
  FPrintWhenExpression := TfrPrintStrings.Create;
  FDesignOptions       := [];
  FPrintOptions        := [poPRepeatValue];
  FPositionType        := ptRelativeToFixTop ;
...
procedure TfrRect.SetoldBottom(AValue: Integer);
begin
  if FoldBottom=AValue then Exit;
  FoldBottom:=AValue;
end;

procedure TfrRect.SetoldLeft(AValue: Integer);
begin
  if FoldLeft=AValue then Exit;
  FoldLeft:=AValue;
end;

procedure TfrRect.SetoldRight(AValue: Integer);
begin
  if FoldRight=AValue then Exit;
  FoldRight:=AValue;
end;

procedure TfrRect.SetoldTop(AValue: Integer);
begin
  if FoldTop=AValue then Exit;
  FoldTop:=AValue;
end;
...
begin
  ...
  fTop    := aValue.Top;
  foldLeft   := aValue.Left;
  foldRight  := aValue.Right;
  foldBottom := aValue.Bottom;
  foldTop    := aValue.Top;
end;
...
begin
  ...
  if  Rc.Top > Margins.oldTop then
      mt:= Margins.oldTop
  else
      mt:= Rc.Top;

  if  Rc.Left > Margins.oldLeft then
      ml:= Margins.oldLeft
  else
      ml:= Rc.Left;

  if  Rc.Right > Margins.oldRight then
      mr:= Margins.oldRight
  else
      mr:= Rc.Right;

  if  Rc.Bottom > Margins.oldBottom then
      mb:= Margins.oldBottom
  else
      mb:= Rc.Bottom;

  XML.SetValue(Path+'PgSize/Value'{%H-}, PgSize);
  XML.SetValue(Path+'Margins/left/Value'{%H-}, ml);
  XML.SetValue(Path+'Margins/Top/Value'{%H-}, mt);
  XML.SetValue(Path+'Margins/Right/Value'{%H-}, mr);
  XML.SetValue(Path+'Margins/Bottom/Value'{%H-}, mb);
