...
{$define ppaint}
uses
  Classes, SysUtils, Types, mysql55conn, mysql56conn, LazFileUtils, LazUTF8,
  LMessages, Forms, Controls, Graphics, Dialogs, ComCtrls, ExtCtrls, Buttons,
  StdCtrls, Menus, typinfo,

  LCLType,LCLIntf,LCLProc,GraphType,Printers, ActnList, Spin,

  ObjectInspector, PropEdits, GraphPropEdits, ECRuler, LR_IFlds,
  
  LR_Class,  LR_Color,LR_Edit, lr_design_ins_filed;
...
type
  ...
  TfrReportUnits = (ruPixels, ruMM, ruInches, ruCM);
  ...
  { TfrObjectInspector }
   TfrObjectInspector = Class(TPanel)
  private
    FPropertyEditorHook : TPropertyEditorHook;
    FSelectedObject     : TObject;
    fPropertyGrid       : TCustomPropertiesGrid;
    fPropertyGridEvent  : TCustomPropertiesGrid;
  ...
  TfrDesignerForm = class(TfrReportDesigner)
     ...
     Align8: TToolButton;
     Align9: TToolButton;
     BtZoomIn: TToolButton;
     BtZoomOut: TToolButton;
     C2: TComboBox;
     C3: TComboBox;
     ClB1: TSpeedButton;
     ClB2: TSpeedButton;
    ...
    PgB3: TSpeedButton;
    PgB4: TSpeedButton;
    PopAligTools: TPopupMenu;
    PopEstanTools: TPopupMenu;
    PopRectTools: TPopupMenu;
    PopTextTools: TPopupMenu;
    Popup1: TPopupMenu;
    Popup2: TPopupMenu;
    PstB: TSpeedButton;
    RedoB: TToolButton;
    ...
    frDock1: TPanel;
    frDock2: TPanel;
    ToolBarAlig: TToolBar;
    ToolBarZoom: TToolBar;
    ToolBarRect: TToolBar;
    ToolBarStand: TToolBar;
    ToolBarText: TToolBar;
    ...
    procedure Ali1Click(Sender: TObject);
    procedure AliCloseClick(Sender: TObject);
    ...
    procedure MLeftResize(Sender: TObject);
    procedure MRigthResize(Sender: TObject);
...
//    procedure InsertDbFields;
  ...
  private
    ...
    FZoomPage: Boolean;
    procedure SetZoomPage(AValue: Boolean);
  ...
  public
    ...
    property ZoomPage  : Boolean read FZoomPage write SetZoomPage;
  end;
...
uses
  ...
  LR_BTyp, LR_Utils, LR_GrpEd, LR_About, LR_DBRel,LR_DBSet,
  DB, IniFiles, LR_DSet, math;
...
var
  ...
  var
    i, j: Integer;
    Re: TRect;
    x,z,zz,xx,sz, mr,ml,mt,mb : Integer;
    w,y,wy,xp,cm,gs,pr    : Variant;
  begin
    with FDesigner.Page.Margins do
    begin
      mr:= Right;
      ml:= Left;
      mt:= Top;
    ...
    pr  :=  FDesigner.per;
    gs  := (FDesigner.GridSize);

    cm  := (FR_U_CM * pr); // Equivalente a un centimetro en pixeles

    with Canvas do
    begin
      if FDesigner.ShowGrid  then
        begin
      ...
      if FDesigner.ShowGrid then begin
        case FDesigner.Units of
          ruCM     :
            begin
              x := 0;
              z := 0;
              ...
              (*
                Cantidad de lineas a dibujar dentro de cada centrimetro
                de la grilla de forma vertical
              *)
              y := cm/gs;
              ...
              for x := 1 to w do begin
                z  := (y*x); // Posición de cada linea
                if (z > ml) and (z < (Self.Width - mr)) then
                  begin
                   Pen.Style   := psSolid;
                   Pen.Color   := $00EFEFEF;
                   Pen.Width   := 1;
                   Line(z,Self.Height-mb,z,mt);
                  end;
              end;

              (*
                 Cantidad de lineas a dibujar que determinan
                 el área de cada centimetro forma vertical
              *)

              w := (wy/cm)-1;

              for x := 1 to w do begin
                z  := (cm*x); // Posición de cada linea
                if (z > ml) and (z < (Self.Width - mr)) then
                  begin
                    Pen.Style   := psSolid;
                    Pen.Color   := $00E1E1E1;
              ...
              (*
                Cantidad de lineas a dibujar dentro de cada centrimetro
                de la grilla de forma horizontal
              *)
              ...
              for x := 1 to w do begin
                z := (y*x);
                if (z > mt) and (z < (Self.Height - mb)) then
                 begin
                  Pen.Style   := psSolid;
                  Pen.Color   := $00EFEFEF;
                  Pen.Width   := 1;
                  Line(Self.Width-mr,z,ml,z);
                end;
              end;

              (*
                 Cantidad de lineas a dibujar que determinan
                 el área de cada centimetro forma horizontal
              *)
         ...
         ruMM     :
           begin
              ...
              (*
                Cantidad de lineas a dibujar para cada milimetro
                de la grilla de forma vertical
              *)
              y := FR_U_MM * pr;
              ...
              *)
              ...
              for x := 1 to w do begin
                z  := (FR_U_CM*x); // Posición de cada linea
                if (z > ml) and (z < (Self.Width - mr)) then
                  begin
                    Pen.Style   := psSolid;
                    Pen.Color   := $00E1E1E1;
         ...
         ruInches :
            begin
              ...
              *)
              ...
              for x := 1 to w do begin
                z  := ((FR_U_INCH * pr)*x); // Posición de cada linea
                if (z > ml) and (z < (Self.Width - mr)) then
                  begin
                    Pen.Style   := psSolid;
                    Pen.Color   := $00E1E1E1;
...
procedure BDownTool(SB: TToolButton);
begin
  SB.Down := True;
end;

procedure BUpTool(SB: TToolButton);
begin
  SB.Down := False;
end;
...
begin
  ...
  with lrFieldsList do
  begin
    Parent          := MRight;
    Align           := alClient;
   btnVariables.OnClick  := @N42Click;
  end;
...
//  FirstInstance := FirstInst;
  PageView        := TfrDesignerPage.Create(Self);
  ...
  begin
    ...
    Items.Add('25%');
    Items.Add('10%');
    Items.Add(sPreviewFormPW);
    Items.Add(sPreviewFormWhole);
  end;

  ZoomCb.Hint     := sPreviewFormScale;
  BtZoomIn.Hint   := sPreviewFormZoomP;
  BtZoomOut.Hint  := sPreviewFormZoomM;
end;
...
procedure TfrDesignerForm.CoolBar1Change(Sender: TObject);
begin
  CoolBar1.AutosizeBands;
end;

procedure TfrDesignerForm.E1Change(Sender: TObject);
begin
  DoClick(E1);
end;
...
begin
  ...
  EditorForm.btnScript.OnClick := nil;
  EditorForm.btnScript.Down    := true;
  EditorForm.btnScript.OnClick := @EditorForm.btnScriptClick;
...
begin
  if CurReport is TfrCompositeReport then
     Exit;
...
procedure TfrDesignerForm.Ali1Click(Sender: TObject);
var
  t : Integer = 0;
  procedure posTool(c : Array of TWinControl; i : Integer; p : Integer  );
  begin
    with c[i] As TToolBar do
    begin
       case p of
         0 : c[i].Align:= alTop ;
         1 : c[i].Align:= alBottom ;
         2 : c[i].Align:= alRight ;
         3 : c[i].Align:= alLeft ;
       end;
    end;
...
begin
  with Sender As TMenuItem do
    begin
      posTool([ToolBarStand,ToolBarText,ToolBarRect,ToolBarAlig],
      Parent.Owner.Tag, Tag);
    end;
end;

procedure TfrDesignerForm.AliCloseClick(Sender: TObject);
  procedure HideTool(c : Array of TWinControl; i : Integer; b : Boolean);
  begin
    with c[i] As TToolBar do
    begin
       c[i].Visible:= b;
       CoolBar1.Bands.Items[i].Visible:= b;
    end;
  end;
...
begin
  with Sender As TMenuItem do
  begin
     HideTool([ToolBarStand,ToolBarText,ToolBarRect,ToolBarAlig], Parent.Owner.Tag, false);
  end;
end;

procedure TfrDesignerForm.BtZoomInClick(Sender: TObject);
begin
  Page.ZoomPage:= True;
...
procedure TfrDesignerForm.BtZoomOutClick(Sender: TObject);
begin
  Page.ZoomPage:= True;
...
begin
  ...
  FilePreview.Enabled := not (CurReport is TfrCompositeReport);
  ClearUndoBuffer;
  ClearRedoBuffer;
  Modified := False;
  ...
  ShowPosition;
  RestoreState;
  ObjInsp.Visible:= True;
  FormResize(nil);
  FileBtn2.Enabled:= True;
    ...
    begin
      ...
      begin
        ...
        if c1 is TToolButton then
           TToolButton(c1).Enabled:= FGrayedButtons;
      end;
    end;
  end;
begin
  FGrayedButtons := Value;
  DoButtons([ToolBarStand,ToolBarText,ToolBarRect,Panel4,ToolBarAlig]);
end;
...
begin
  Result := x;
  case FUnits of
    ruMM      : Result := x / FR_U_MM;
    ruCM      : Result := x / FR_U_CM;
    ruInches  : Result := x / FR_U_INCH;
  end;
end;
...
begin
  Result := Round(x);
  case FUnits of
    ruMM      : Result := Round(x * FR_U_MM);
    ruCM      : Result := Round(x * FR_U_CM);
    ruInches  : Result := Round(x * FR_U_INCH);
  end;
end;
...
procedure TfrDesignerForm.ZoomPageUotPut;
var
  xw, wh,wt,wl, mr, ml,mb,mt : Variant;
  i : Integer;
  t : TfrView;
begin
  {if Page = Nil then
  ...
  begin
    ...
    xw             := (Page.PrnInfo.Pgw * per );
    wh             := (Page.PrnInfo.Pgh * per);
    Page.Width     := xw ;
    Page.Height    := wh ;
    with Page.Margins do
    begin
   ...
   { for i := 0 to Pred(Objects.Count) do begin
       t  := TfrView(Objects[i]);
       with t do
       begin
        wt := (y / BeforeScale);
  ...
  ScrollBox1.HorzScrollBar.Range := ScrollBox1.HorzScrollBar.Range + 15;
  if per = 1 then
     BeforeScale:= 1;  }
end;

procedure TfrDesignerForm.SetScale;
var
  dwx, dwy : Integer;
begin
  PaintAllowed:= False;
  ...
  case mode of
    mdNone        :;
    mdPageWidth   : per := (panTab.Width - 100) / dwx;
    mdOnePage     : per := (panTab.Height - 60) / dwy;
  end;

  ZoomCb.Text     := IntToStr(Round(per * 100)) + '%';
  LastScale       := per;
  LastScaleMode   := mode;
end;
...
procedure TfrDesignerForm.RefreshListDataVar;
begin
  with lrFieldsList do
  begin
     RefreshDSList;
...
procedure TfrDesignerForm.SetZoomPage(AValue: Boolean);
begin
  if FZoomPage=AValue then Exit;
  FZoomPage:=AValue;
end;
...
begin
  {$IFDEF DebugLR}
  debugLnEnter('TerDesignerForm.SelectionChanged INIT, SelNum=%d',[SelNum]);
  {$ENDIF}
  HideIEButton;
  Busy := True;
  ColorSelector.Hide;
  ...
  ActiveControl := nil;
  {$IFDEF DebugLR}
  debugLnExit('TerDesignerForm.SelectionChanged END, SelNum=%d',[SelNum]);
  {$ENDIF}
end;
...
procedure TfrDesignerForm.ZoomCbChange(Sender: TObject);
var
  i : Integer;
  c : String;
begin
...
begin
  ...
  begin
    ...
    E3.Text := IntToStr(Margins.Left/ c);
    E4.Text := IntToStr(Margins.Top/ c);
    E5.Text := IntToStr(Margins.Right/ c);
    E6.Text := IntToStr(Margins.Bottom/ c);
    E7.Text := IntToStr(ColGap / c);
    ...
    begin
      ...
      try
        Margins.AsRect := Rect(StrToInt(E3.Text) * c,
                          StrToInt(E4.Text) * c,
                          StrToInt(E5.Text) * c,
                          StrToInt(E6.Text) * c);
        ColGap         := StrToInt(E7.Text) * c;
      except
        on exception do
        begin
          Margins.AsRect := Rect(FR_U_CM, FR_U_CM, FR_U_CM, FR_U_CM);
          ColGap         := 0;
        end;
      end;
...
procedure TfrDesignerForm.MLeftResize(Sender: TObject);
begin
  FormResize(Sender);
end;

procedure TfrDesignerForm.MRigthResize(Sender: TObject);
begin
  FormResize(Sender);
end;
...
begin
  ...
  if Assigned(lrFieldsList) and
    ((Control = lrFieldsList.TreeVars) or (Control = lrFieldsList.TreeDB)) then
  begin
    with lrFieldsList do begin
      if PageControl1.ActivePageIndex = 0 then begin
         if HeaderCB.Checked then
         begin
            t := frCreateObject(gtTextArea, '', Page) as TfrCustomMemoView;
            t.MonitorFontChanges;
            t.Memo.Text:=UpperCase(lrFieldsList.SelectedText);
...
begin
  Accept:= false;
  if Page is TfrPageDialog then
     Exit;
  Control:=lrDesignAcceptDrag(Source);
  if Assigned(lrFieldsList) then
    Accept:= (Control = lrFieldsList.TreeVars)
    or (Control = lrFieldsList.TreeDB);
end;
...
procedure TfrDesignerForm.Splitter2Moved(Sender: TObject);
begin
  FormResize(Sender);
end;
...
begin // toolbars
 Pan1.Checked := ToolBarRect.IsVisible;
 Pan2.Checked := ToolBarStand.IsVisible;
 Pan3.Checked := ToolBarText.IsVisible;
 Pan4.Checked := Panel4.IsVisible;
 Pan5.Checked := MLeft.Visible;
 Pan6.Checked := ToolBarAlig.Visible;
 Pan7.Checked := Panel6.Visible;
 MenuItem2.Checked:= MRight.Visible;
end;
  ...
  begin
    if c[i] is TPanel then
      ...
      begin
        ...
        case i of
          0 : CoolBar1.Bands.Items[0].Visible:= b ;
          1 : CoolBar1.Bands.Items[1].Visible:= b;
          2 : CoolBar1.Bands.Items[2].Visible:= b;
          4 : CoolBar1.Bands.Items[3].Visible:= b;
        end;
      end;
    FormResize(Sender);
  end;
begin // each toolbar
  ...
  begin
    Checked := not Checked;
    SetShow([ToolBarStand,ToolBarText,ToolBarRect,Panel4,ToolBarAlig,MLeft,Panel6,MRight], Tag, Checked);
  end;
end;
...
begin
  ...
  DoSaveToolbars([MLeft, Panel4, Panel6]);
...
procedure TfrDesignerForm.SetMenuItemBitmap(AMenuItem: TMenuItem;
  ABtn: TSpeedButton);
var
  m: TfrMenuItemInfo;
begin
...
begin
  ...
  if GridSize = 18 then
    dy := 18;
  if LastFontSize in [12, 13] then
     dy := 20;
  if LastFontSize in [14..16] then
     dy := 24;
end;
...
{//**
  if MLeft.IsFloat then SetWinZOrder(MLeft.FloatWindow);
...
procedure TfrObjectInspector.SetPropertyEditorHook(AValue: TPropertyEditorHook);
begin
...
begin
  ...
  tpPage      := TPageControl.Create(Self);
  with tpPage do
  begin
    Parent    := Self;
    Align     := alClient;
    AddTabSheet.Caption:= 'Propiedades';
    AddTabSheet.Caption:= 'Eventos';
  end;
  ...
  begin
    ...
    Align                    := alTop;
    ShowHint                := True; //cause problems in windows
  ...
  begin
    Name                       :=  'PropertyGrid';
    Parent                     :=  tpPage.Pages[0];
    Align                       := alClient;
    fPropertyGrid.SaveOnChangeTIObject := false;
    DefaultItemHeight                  := fcboxObjList.Height-4;
    Color                              := clWhite;
    Font.Color                         := clBlack;
    Font.Size                          := 8;
    Filter                             := PROPS;
    ...
    SubPropertiesColor                 := $00620000 ;
    ReferencesColor                    := $007C7C7C;
    DrawHorzGridLines                  := True;
  end;

   fPropertyGridEvent:=TCustomPropertiesGrid.Create(aOwner);
  with fPropertyGridEvent do
  begin
    Name                        := 'PropertyGridEvent';
    Parent                      := tpPage.Pages[1];
    Align                       := alClient;
    ShowHint                    := True; //cause problems in windows
    fPropertyGridEvent.SaveOnChangeTIObject := false;
    DefaultItemHeight                  := fcboxObjList.Height-4;
    Color                              := clWhite;
    Font.Color                         := clBlack;
    Font.Size                          := 8;
  ...
  splitBot := TSplitter.Create(Self);
  with splitBot do
  begin
    Parent      := Self;
    Align       := alBottom;
...
begin
  ...
  begin
    ...
    try
      ...
      fPropertyGridEvent.Selection := NewSel;
    finally
      NewSel.Free;
    end;
  end
...
begin
  ...
  //fPropertyGridEvent.OnModified:=AEvent;
end;
...
begin
  if not visible then
    exit;
  fPropertyGrid.Filter:= PROPS;
  fPropertyGrid.RefreshPropertyValues;
  fPropertyGridEvent.Filter:= [tkMethod];
  fPropertyGridEvent.RefreshPropertyValues;
end;
...
begin
  ...
  begin
    if (DataSet=nil) or (TreeDB.Items.Count = 0) or (TreeDB.Selected.Index = -1)
       or (TreeDB.Selected.Level = 1) or (TreeDB.Selected.Level = 0) then
      exit;
    ...
    try
      ...
      TfrDesignerForm(frDesigner).SelNum := 0;
      if (TreeDB.Selected.Selected) then begin
        if TreeDB.Selected.Level > 1 then
        begin
          f := TfrTField(DataSet.FindField(TreeDB.Selected.Text));
          fSize := 0;
          if f <> nil then
            begin
            ...
            begin
              f1    := DataSet.FieldDefs[TreeDB.Selected.Index];
              fSize := f1.Size;
              fName := f1.Name;
            end;
          ...
          if HeaderCB.Checked then
            ...
            begin
              t.Memo.Add('[' + TreeDB.Selected.Parent.Text +
                '."' + TreeDB.Selected.Text + '"]');
...
//          TfrDesignerForm(frDesigner).Page.Objects.Add(t);
          ...
          if HeaderCB.Checked then begin
